...
export default function NotificationCard({data,subject}) {
    ...
    return (
        <div className='tw-flex tw-flex-col tw-py-1 tw-px-2 tw-bg-[white] tw-m-1 tw-rounded-sm tw-cursor-pointer tw-text-black hover:tw-bg-slate-100 tw-transition-all tw-duration-300'>
            <div>{data.body}</div>
            <div className='tw-text-right tw-text-sm tw-text-gray-600'>{subject} {getTime(data.timestamp)}</div>
        </div>
    )
}
